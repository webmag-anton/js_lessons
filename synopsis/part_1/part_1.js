...
// с появлением ES5 (2009 г.) в JS появилась дирректива 'use strict', которая позволяет использовать 
// новые возможности языка, но при этом не работают старые, изжившие себя (ошибки или несовершенные 
// решения). Для обратной совместимости не используют 'use strict'. То есть, чтобы устаревший код 
// работал, как и раньше, не используют 'use strict'. А для включения новых возможностей ES5+ 
// используют 'use strict'. Классы и модули автоматически включают строгий режим, поэтому в React 
// компонентах нет нужды в 'use strict'
...
arr.slice(start, end) - возвращает новый подмассив, без вырезания; не изменяет исходный массив
                        можно использовать arr.slice() без аргументов для не глубокого клонирования arr
...
Эти специальные методы ведут себя как св-ва снаружи объекта/класса
let obj = {
   get propName() {
...
Класс - продвинутый способ создания обектов, вместо обычного конструктора; Классы еще называют абстракциями;
методы класса попадают в его свойство prototype, откуда наследуются; после свойств и методов запятая не ставится;

// Владилен говорит, что хорошей практикой считается задание сначало полей, затем конструктора, а потом методов 
class MyClass {
  ...
  // свойство не из constructor() так же присваивается создаваемому экземпляру, но с фиксированным value
  prop = value 
...
getTime() - получить timestamp – количество миллисекунд, прошедших с 1 января 1970 года UTC+0
...
setSeconds(sec, [ms])
setMilliseconds(ms)
setTime(ms) - установить timestamp - количество миллисекунд, прошедших с 1 января 1970 года UTC+0
...
// Date.now() это менее затратный эквивалентен new Date().getTime()
...
JSON.stringify() - для преобразования {сериализации} поддерживаемых значений в JSON (становится строкой)
