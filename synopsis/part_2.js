...
// Можно использовать Array.from() для создания массива из коллекции
...
дополнительное св-во relatedTarget; оно содержит ссылку на элемент с/на который мы переходим
mouseenter/mouseleave - вызывается, когда курсор переходит на элемент или уходит с него; но
переходы внутри элемента на его потомки и с них не считаются; так же есть св-во 
relatedTarget; эти события не всплывают, поэтому нельзя использовать делегирование!
...
// стилей, т.к. скрипту может понадобиться получить свойства элементов, зависящих от стилей
...
unload – пользователь покидает страницу {обычно здесь отсылают статистику}; отменить переход на 
