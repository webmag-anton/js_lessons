...
// рендеринг страницы происходит постепенно по мере нисходящего 
// парсинга документа и выполнения Critical Rendering Path !!!

//   рендеринг страницы начинается только после Critical Rendering Path: 
// 1. построения DOM, на которое влияют:
//     - загрузка и парсинг HTML узлов;
//     - загрузка и выполнение всех скриптов (включая внутренние/внешние, 
//       кроме defer/async и динамически сгенерированных), стоящих над 
//       выше-распаршенными HTML узлами;
// 2. построения CSSOM (CSS Object Model) после загрузки (если внешний) и
//    парсинга стилей, стоящих над выше-распаршенными HTML узлами
// 3. формирования дерева рендера (render tree) на основе DOM и CSSOM
// 4. компановки объектов (layout/flow): позиционирование на странице
//    элементов из render tree

// * стоит понимать что загрузка и выполнение внешних рессурсов (стили, картинки, 
// js...) не блокирует нисходящий парсинг документа => все внешние нижестоящие 
// рессурсы в документе будут загружаться параллельно примерно с одной скоростью;
// * т.к. картинки не блокируют рендеринг страницы, они могут рендериться после 
// отрисовки документа => для них нужно предусмотреть placeholder, что бы не было 
// скачков при reflow (перепозиционировании) из-за появления размеров картинок; 
// если размеры для картинок заданы в стилях, то reflow не будет;
// * выполнение внутреннего скрипта (т.е. внутри <script>) блокирует рендеринг как
// вышестоящего так и нижестоящего html!



      // Очередность загрузки и выполнения скриптов при парсинге HTML-документа:

//     <script [src]>           ->        <script defer>        ->            DOMContentLoaded              ->           load  
// (ждет загрузки и парсинга          выполняются после               (событие у document)                        (событие у window) 
// стилей, стоящих перед ним          всех <script [src]>,            наступит когда первоначальный HTML          браузер загрузил HTML и
// и только после этого               но до DOMContentLoaded          документ будет полностью загружен           все внешние рессурсы 
// выполняется скрипт)                                                и распаршен, без ожидания загрузки          (картинки, стили, любые
//                                                                    всех стилей, картинок и фреймов.            скрипты (включая async и  
//                                                                    Ждет только выполнения всех                 динамически загруженные))                
//                                                                    скриптов (кроме async и динамически   
//                                                                    загруженных) и всех внешних стилей 
//                                                                    перед этими скриптами 

// * сначала выполнятся все внутренние/внешние скрипты, а потом скрипты с defer 
// * есть кроссбраузерный хак для внешнего стиля, чтобы его загрузка не блокировала  
// скрипт за ним и рендеринг документа (стиль примениться после его загрузки): 
// <link rel='stylesheet' href='./style.css' media='print' onload='this.media="all"'>
// * сами по себе стили не влияют на событие DOMContentLoaded, т.к. они не могут 
// изменить DOM; напимер, display: none не удаляет node-элемент, он просто не 
// отображается, а :before не является node-элементом (это псевдоэлемент);
// на DOM может повлиять только скрипт (удалить, переместить узел...), но скрипт
// должен дождаться загрузки стоящих выше стилей, т.к. скрипту может понадобиться
// получить свойства элементов, зависящие от стилей
