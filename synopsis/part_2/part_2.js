...
// парсер ждет загрузки               (ждет загрузки внешних       (ждет формирования DOM,       (событие у document)            (событие у window) выполнится
// и выполнения скриптов              стилей (<link href=''),      на который влияет             после построения DOM +          после отрисовки страницы включая
// без defer для продолжения          стоящих перед ним и          загрузка (+ выполнение)       выполнения всех                 загрузку и отрисовку всех
// нисходящего формирования           только после этого           всех скриптов без defer       скриптов, включая defer, и      внешних рессурсов (картинки, 
// DOM, при этом загрузка             выполняется скрипт)          и внешних стилей перед        всех внешних стилей перед       стили...); возможно внешние  
// внешних рессурсов (картинки,                                    этими скриптами, если         скриптами без defer;            рессурсы загрузятся раньше 
// стили...) не блокирует парсер                                   они есть)                     только после выполнения         события DOMContentLoaded 
// (внешние рессурсы начинают                                                                    callback'a на событие 
// загружаться когда до каждого                                                                  DOMContentLoaded начинает
// из них доходит парсер, не                                                                     рендериться страница !
// блокируя друг друга)
...
// поэтому картинки могут рендериться после разметки документа => для них нужно 
// предусмотреть placeholder, что бы не было скачков из-за появления размеров картинок;
// если размер для картинок заданы стилями, то скачков не будет

// * динамические скрипты (созданые при помощи document.createElement('script') и 
// затем добавленные на страницу) не блокируют DOMContentLoaded, они выполняются 
// после DOMContentLoaded не зависимо от load у window

// * есть кроссбраузерный хак для применения тяжелого стиля, чья загрузка не блокирует  
// скрипт, идущий за ним (стиль может применить после рендеринга страницы): 
