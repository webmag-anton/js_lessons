...
// когда браузер парсит HTML-документ и встречает тег <script>, он должен выполнить его перед тем, 
// как продолжить строить DOM (т.е. скрипт блокирует парсинг документа); это делается на случай, 
// если скрипт захочет изменить DOM; поэтому DOMContentLoaded случится после таких скриптов
...
//   и затем добавленные на страницу, также не блокируют DOMContentLoaded

// скрипты дожидаются загрузки внешних стилей, стоящих перед ними, а DOMContentLoaded ждет скрипты!
// DOMContentLoaded не ждет загрузки внешних стилей; но если после внешних стилей у нас есть обычный
...
load {window} – событие на window, когда браузер загрузил HTML {DOMContentLoaded} и 
                внешние ресурсы {картинки, стили и тд}
...
- скрипты с defer никогда не блокируют страницу {ждут окончания парсинга HTML, полного формирования 
  DOM, а также подгрузки остальных обычных скриптов {не важно внешних или нет} без defer}
...
//   Critical Rendering Path (критические этапы рендеринга) - это последовательность шагов, 
// которые выполняет браузер с помощью своих движков (WebKit, Blink, Gecko...), когда 
// преобразуется HTML, CSS и JavaScript в пиксели, которые вы видите на экране. Эти шаги 
// включают в себя работу с Document Object Model (DOM), CSS Object Model (CSSOM), деревом 
// рендера (render tree) и компоновкой объектов (layout)

//      рендеринг страницы начинается только после Critical Rendering Path: 
// 1. построения DOM (происходит событие DOMContentLoaded), на которое влияют:
//     - загрузка и парсинг всех HTML узлов;
//     - загрузка и выполнение всех скриптов (включая inline/внешние и с defer), 
//       кроме async и динамически сгенерированных скриптов;
//     - все внешние стили перед скриптами без defer;
// 2. построения CSSOM (CSS Object Model), которое происходит после загрузки 
//    и парсига всех стилей, подключенных к документу, не важно внеших или нет
// 3. формирования дерева рендера (render tree) на основе DOM и CSSOM
// 4. компановки объектов (layout/flow): позиционирование на странице элементов 
//    из render tree

// * стоит понимать что загрузка стилей не блокирует парсинг документа, но блокирует 
// рендеринг страницы, т.к. для рендеринга нужно применить все стили к элементам;
// * с картинками другая ситуация, они не блокируют ни парсинг ни рендеринг страницы,
// поэтому картинки могут рендериться после рендеринга документа => для них нужно 
// предусмотреть placeholder, что бы не было скачков при reflow (перепозиционировании) 
// из-за появления размеров картинок; если размеры для картинок заданы в стилях, то 
// reflow не будет;
// * скрипты с async и динамически сгенерированные скрипты не блокируют рендеринг 
// страницы, в отличае от всех остальных
// ---
// Таким образом: рендеринг страницы может начаться и до загрузки картинок и выполнения
// async/dynamic scripts, но всегда после выполнения всех стилей и js (кроме async/dynamic)


      // Очередность загрузки и выполнения скриптов при парсинге HTML-документа:

//        DOM                    ->       <script [src]>       ->     <script defer>   ->       DOMContentLoaded          ->         load  
// парсер ждет загрузки               (ждет загрузки внешних        выполняется перед      (событие у document)               (событие у window) 
// и выполнения скриптов              стилей (<link href=''),       DOMContentLoaded       запускается когда                  браузер загрузил HTML и
// без defer для продолжения          стоящих перед ним и                                  первоначальный HTML документ       все внешние рессурсы 
// нисходящего формирования           только после этого                                   будет полностью загружен и         (картинки, стили, любые
// DOM, при этом загрузка             выполняется скрипт)                                  распаршен, без ожидания            скрипты (включая async и  
// внешних картинок, стилей                                                                загрузки всех стилей,              динамически загруженные))
// не блокирует парсер (внешние                                                            картинок и фреймов.                
// рессурсы начинают загружаться                                                           Ждет только выполнения всех        
// когда до каждого из них                                                                 скриптов (кроме async и            
// доходит парсер, не блокируя                                                             динамически загруженных),          
// друг друга); выполнение js                                                              включая defer, и всех внешних      
// не блокирует загрузку рес-ов                                                            стилей перед скр-ми без defer 


// * динамические скрипты (созданые при помощи document.createElement('script') и 
// затем добавленные на страницу) не блокируют DOMContentLoaded и рендер страницы,
// т.к. они по умолчанию ведут себя как срипты с async
// * есть кроссбраузерный хак для применения внешнего стиля, чья загрузка не блокирует  
// скрипт за ним и рендеринг страницы (стиль может примениться после рендеринга страницы): 
// <link rel='stylesheet' href='./style.css' media='print' onload='this.media="all"'></link>
...
событий, setTimeout, setInterval называют «macrotask queue» ( термин v8)

Очередь, которую формируют обработчики промисов, async/await и MutationObserver 
называют «microtask queue». Микрозадачи исполняются непрерывно одна за другой. 
У микрозадач приоритет над макрозадачами, они выполняются как можно быстрее!!!

Например, в консоле будет такая последовательность: 1 5 3 4 2
...
2) Исполнить все микрозадачи из очереди microtask queue c отрисовкой изменений (если 
   они есть) страницы для каждй задачи.
3) Если очередь макрозадач пуста, подождать, пока появится макрозадача.
4) Перейти к шагу 1.
...
count(); 
  ...
  Метод requestIdleCallback() (у window) ставит в очередь функцию, которая будет вызываться 
во время периодов простоя браузера

// возвращает ID, который может быть использован для отмены 
// callback'а, передав значение в метод cancelIdleCallback()
var handle = window.requestIdleCallback(callback[, options])

callback - ссылка на функцию, что должна быть вызвана в ближайшем 
           будущем, когда цикл событий (event loop) в режиме ожидания
